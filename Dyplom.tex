...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\Mastertrue % INFO: odkomentuj, jeli to praca magisterska
...
\specjalnosc{Grafika i systemy multimedialne (IGM)} % INFO: nazwa specjalnoci
\promotor{Prof. uczelni, dr hab. inż. Henryk Maciejewski} % INFO: dane promotora 
\kvpl{detekcja obiektów, przetwarzanie obrazów, rozpoznawanie obrazów} % INFO: s³owa kluczowe po polsku
\kven{object detection, image processing, image recognition} % INFO: s³owa kluczowe po angielsku
\date{WROCŁAW, 2024} % INFO: miejscowoæ, rok z³o¿enia pracy dyplomowej
...
%       tych czêci (plików z latexowym kodem), które maj¹ byæ kompilowane. 
%       Przydaje siê to szczególnie podczas pracy nad du¿ymi dokumentami. 
%       Bo im mniej czêci zostanie wyselekcjonowanych, tym szybsza bêdzie kompilacja.
...
\maketitle
\clearpage
% INFO: Strona z licencj¹ CC zdefiniowana poni¿ej dotyczy tylko zredagowanej treci. 
%       Z samego szablon mo¿na korzystywaæ bez koniecznoci cytowania jego autora.
%       Podczas pisania pracy dyplomowej stronê tê mo¿na usun¹æ (a nawet nale¿y).

% Kolejne czêci dokumentu: streszczenie, spisy, skróty, rozdzia³y, dodatki
%\chapterstyle{noNumbered}
% STRESZCZENIE (proszê zajrzeæ do rodka na zakomentowane komendy)
\include{streszczenie}
\pagestyle{outer}
\clearpage
% SPIS TRECI (zostanie wygenerowany automatycznie)
\pdfbookmark[0]{Spis treci}{spisTresci.1}%
%%\phantomsection
%%\addcontentsline{toc}{chapter}{Spis treci}
\tableofcontents* 
\clearpage
% SPIS RYSUNKÓW (zostanie wygenerowany automatycznie)
\pdfbookmark[0]{Spis rysunków}{spisRysunkow.1} % jeli chcemy mieæ w spisie treci, to zamarkowaæ tê liniê, a odmarkowaæ linie poni¿sze
%%\phantomsection
...
\lstlistoflistings*
\clearpage
% SKRÓTY (to opcjonalna czêæ pracy)
% ROZDZIA£Y (kolejne rozdzia³y do³¹czane s¹ z kolejnych plików)
\chapterstyle{default}
\include{r_wstep/_wstep}
\include{r_technologie/_uzyteTechnologie}
\include{r_implementacja/_main}
\include{r_test_szybkosci/_main}
\include{r_test_dokładności/_main}
\include{r_podsumowanie/_podsumowanie}
...
\setlength{\bibitemsep}{2pt} % - zacienia wykaz
%       Pozycja Literatura pojawia siê w spisie treci nieco inaczej ni¿ spisy rysunków, tabel itp.
%       Aby zachowaæ w³aciwe odstêpy nale¿y u¿yæ poni¿szej komendy
\addtocontents{toc}{\addvspace{2pt}} % ustawiamy odstêp w spisie treci przed pozycj¹ Literatura 
...
% Jeli w pracy pojawiaæ siê ma indeks, nale¿y odkomentowaæ poni¿sze linie
%%\chapterstyle{noNumbered}
